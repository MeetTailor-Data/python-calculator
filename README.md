# Calculator Application (Python)

A **menu-driven Python calculator application** that performs basic arithmetic operations.
This project is designed using **core Python concepts** with a simple and clear structure, making it ideal for beginners who are learning Python programming.

---

## Project Description

The calculator allows users to perform arithmetic operations such as **addition, subtraction, multiplication, and division** through a terminal-based menu.
The program runs continuously until the user chooses to exit.

---

## Features

* Menu-driven interface
* Separate functions for each operation
* Continuous execution using a loop
* Handles division by zero safely
* User-friendly input and output
* Clean and readable code structure

---

## Concepts Used

### Data Types

* Integers
* Floating point numbers
* Strings
* Lists
* Dictionaries
* Booleans

### Control Statements

* `if / elif / else`
* `while` loop

### Programming Concepts

* Functions
* Lambda expressions
* Dictionary-based function mapping
* List for storing calculation history
* Input and output handling
* Basic error handling

---

## Project Structure

```
python-calculator/
│
├── Calculator.py   # Main calculator program
└── README.md       # Project documentation
```

---

## How to Run the Program

### Requirements

* Python 3.x installed on your system

### Steps

1. Open terminal or command prompt
2. Navigate to the project directory
3. Run the program using:

```bash
python Calculator.py
```

### Application Menu

```
1. Addition
2. Subtraction
3. Multiplication
4. Division
5. Exit
```

### Sample Output

```
Enter your choice: 1
Enter first number: 10
Enter second number: 5
Result: 15
```

---

## Edge Cases Handled

* Division by zero
* Invalid menu choices
* Program continues running until the exit option is selected

---

## Future Improvements

* Display calculation history
* Save calculation history to a file

---

## Author

**Meet Tailor**
Python Programming Learner

---

## License

This project is created for learning and educational purposes.

---

## Status

* Project Status: Completed
* Last Updated: 2026
